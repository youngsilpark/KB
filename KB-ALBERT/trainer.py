...
from transformers import AutoModel, AutoTokenizer,AutoModelForTokenClassification,AlbertConfig
...
class Trainer(object):
    def __init__(self, args, train_dataset=None, dev_dataset=None, test_dataset=None):
        ...
        # self.model_config=AlbertConfig().from_pretrained(kb_albert_model_path,)

        kb_albert_model_path = "/content/drive/MyDrive/bigcare/KoBERT-NER/kb-albert-char-base-v2/"
        self.config = AlbertConfig.from_pretrained(kb_albert_model_path,
        ...
        self.model = AutoModelForTokenClassification.from_pretrained(kb_albert_model_path,config=self.config)
        # self.model.config.id2label={str(i): label for i, label in enumerate(self.label_lst)}
        # self.model.config.label2id={label: i for i, label in enumerate(self.label_lst)}
    ...
    def load_model(self):
        ...
        try:
            self.model = AutoModelForTokenClassification.from_pretrained(self.args.model_dir)
            self.model.to(self.device)
